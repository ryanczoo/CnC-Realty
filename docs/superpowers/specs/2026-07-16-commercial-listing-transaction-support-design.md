...
| `apps/web/src/app/(dashboard)/dashboard/transactions/new-transaction/page.tsx` | Add a new "Property Category" `OptionCard` section to Step 0, directly below "Transaction Stage," using the same pattern (Residential/Commercial). Conditionally rendered only when `form.transactionSide` is set and is not `REFERRAL` (mirrors the existing Stage section's conditional). `canAdvance`'s Step 0 check must also require `propertyCategory` for non-Referral sides, so this new field can't be skipped. (This is scoped narrowly to the new field only — the broader "Next doesn't validate any required field" issue Ryan separately flagged is its own, unrelated fix, tracked outside this spec.) |
...
**Resolved (2026-07-16):** the New Transaction wizard's `propertyCategory` UI. It's a new, explicit Residential/Commercial choice — not derived from the existing `propertyType` dropdown (Single Family/Condo/.../Commercial/Land/etc.), because several of those 11 values don't map cleanly to a binary category (is "Land" or "Farm and Ranch" residential or commercial?) and a silent wrong guess would attach the wrong, possibly legally-required, checklist. It lives on **Step 0** ("File Type"), as a third `OptionCard` section directly below "Transaction Stage" — consistent with Step 0 already being the deal's big-picture classification (who's represented, what stage), not a property *detail* like address/MLS#, which belongs on Step 1. It's conditionally shown only when `transactionSide` is set and isn't `REFERRAL` (referral files have no property at all, so no category applies), mirroring exactly how the existing Stage section is conditionally shown today.
...
6. **New Transaction wizard's `propertyCategory` UI: a new, explicit Step 0 selector**, not derived from the existing `propertyType` dropdown — see §5 for full reasoning and placement.
