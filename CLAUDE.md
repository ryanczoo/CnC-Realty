...
3. ~~Decide whether to suppress the homepage title/count on total DB outage~~ — **closed 2026-07-24, no action needed.** Ryan confirmed the LA-fallback-with-placeholder-cards mismatch during a total DB outage is fine as-is: a full outage means bigger problems anyway and agents get notified directly, so guarding against this cosmetic seam isn't worth it.
...
5. Older backlog, unchanged: **"testing the transaction management system"** broader click-through (Purchase/Listing/Lease Tenant-Landlord — still only the referral lifecycle and now the two new listings sections have had live verification); full IDX resync (deliberately deferred, no target date).

**Checklist templates — confirmed DONE (2026-07-24), removed from backlog after appearing stale on this list for 5+ sessions.** Git history shows this was completed well past the original 3-template scope: `0105d5e`/`52030c5` (2026-07-14) replaced placeholder seed names with real, verified C.A.R. forms and closed a gap where some listing/transaction types got zero checklist; `d78c49e` (2026-07-15) is the wizard-label rename Ryan recalled; `dc74090`/`4a78805`/`9d3bf7a`/`2ecbdfa`/`a93e760`/`4f2dd71`/`1f51ef5` (2026-07-16) added a full 8-template commercial set + `propertyCategory` matching. Current `seed.ts` has 14 templates total, covering every residential purchase/listing/lease/dual/referral combination plus commercial.

**The 7 disclosed duplicate-ownership-check files from Task 9 — DONE (2026-07-24).** A fresh grep sweep found the actual current count was 12, not 7 (Task 9's "7" was a same-day side-observation, not an exhaustive search — nothing had changed in the interim, the count was just always higher). All 12 migrated onto the shared `checkOwnership()` helper in `lib/api-auth.ts`, each preceded by a characterization test proving current behavior before touching the code: `campaigns/[id]/contacts`, `campaigns/[id]/send`, `campaigns/[id]/drip-steps`, `deals/[id]/convert`, `listings/[id]/convert`, `leads/[id]/activities`, `leads/[id]/tags/[tagId]`, `leads/[id]/enrollments/[enrollmentId]`, `leads/[id]/relationships/[relId]`, `leads/[id]/tasks/[taskId]` were pure zero-behavior-change refactors. Two — `listings/[id]/submit-review` and `transactions/[id]/submit-review` — got a deliberate, Ryan-approved behavior change: they previously had no ADMIN bypass at all (inconsistent with their sibling `convert` routes); RED tests proved ADMIN got 403 on the old code, then the migration fixed it to 200. Full suite went from 544 → 578 passing, zero regressions, `tsc --noEmit` clean, and a final grep sweep confirms zero bare `agentId !==` ownership comparisons remain anywhere outside `lib/api-auth.ts`.

---

## Session Notes — 2026-07-24 / 2026-07-25 (dashboard speed investigation + fixes)

### What Was Completed

All changes committed on `feature/agent-application-redesign` (5 commits: `74b2241`, `0ab9be0`, `770249a`, `409d406`, `e6ccfda`). Branch now 24 commits ahead of `origin/feature/agent-application-redesign` — **not pushed yet**, Ryan's call when ready.

Also fixed the campaigns tab empty state per Ryan's request: removed the redundant "No campaigns yet." line and the trailing period on "Create your first email campaign" (`74b2241`).

**The checkOwnership migration investigated earlier tonight (see the "7 disclosed files" note directly above) was actually implemented and committed this session** — `0ab9be0`, 24 files, full suite 544→578.

### Dashboard tab-switching investigation

Ryan reported tabs (especially Transactions → Tasks) taking 5-10 seconds to switch, despite the 2026-07-08/09 fix that cached `agentId` on the session. Used `systematic-debugging` to gather real evidence (dev server request logs, direct DB queries, code reads) rather than guessing. Found **four distinct, independent causes**:

1. **Dev-only compile lag** — first hit to each route after a `pnpm dev` restart triggers Next.js's on-demand compilation (5.4s for `/api/transactions`, 2.6s for `/api/tasks`, etc.). Confirmed this disappears entirely on a real deploy (Vercel prebuilds everything) — nothing to fix here.
2. **A real, confirmed bug: stale session `agentId`.** `lib/auth.ts`'s `jwt` callback only ever computed `agentId` at sign-in, never refreshed after. Live DB query confirmed Ryan's ADMIN account has a valid `Agent` record right now, yet the live server log showed `/api/transactions` and `/api/listings` both 404ing with "Agent not found" — meaning his *active session's* JWT was carrying a stale value from before that Agent record existed (or from an earlier lookup that came back empty), and nothing had ever re-checked it since.
3. **No-caching client-fetch architecture** — Transactions/Tasks/Pipeline were all `"use client"` + `useEffect`+`fetch()` with zero caching (no SWR, no React Query), so every tab revisit re-fetched from scratch behind a full loading skeleton, even seconds after the last visit.
4. **Oversized, uncached headshot fetch** — Settings page's `/api/headshot/[userId]` re-fetched a 4.4MB image fresh from R2 every single load (too big for Next's fetch-cache, so caching silently failed every time), and fired twice back-to-back due to React Strict Mode's dev-only double-effect-invocation (~4.5s each, ~9s combined — this one is *not* dev-only, it would cost the same in production).

### Decisions and fixes, in the order discussed

1. **Headshot fix — approved and shipped (`770249a`).** Added `sharp`, resize to max 512px/side + re-encode JPEG quality 82 before `uploadToR2`. Confirmed: storage is R2 object storage, not the database (DB only holds a key string) — so this *reduces* total storage, doesn't increase anything; visually identical since a headshot only ever renders small on-screen.
2. **Client-fetch caching — approved and shipped (`409d406`).** Added `@tanstack/react-query` usage to Transactions/Tasks/Pipeline (the dependency and `QueryClientProvider` already existed in `Providers.tsx`, just never actually used anywhere). Default `staleTime: 0` + `refetchOnMount`/`refetchOnWindowFocus` handles cross-page staleness automatically; same-page mutations (Tasks checkOff, Pipeline drag/drawer/modal) explicitly wired to update the cache. New `lib/dashboard-queries.ts` holds the tested, extracted fetch/cache-update functions. **Bonus fix, a natural side effect of wiring this correctly:** `DealBoard`'s drag-and-drop previously never reported changes back to the parent page at all, so switching pipeline tabs away and back could silently revert a drag — fixed by the new `onDealUpdated` callback.
3. **AgentId stale-session bug — discussed at length, then approved and shipped (`e6ccfda`).** Ryan asked how other sites handle this ("I assumed auto-logout after a while"). Researched via WebSearch rather than assuming: auto-expiring sessions are a *security* control (bounding how long a leaked token stays valid), not a freshness mechanism — a session can sit stale for its entire lifetime and only get corrected incidentally when it finally expires. The patterns that actually solve claim-freshness are short-lived-token-silent-refresh and DB-side "something changed" version flags, both checked on a cadence, not on every request and not via a blanket logout timer. Landed on a **cadence-based re-check**: `jwt` callback now stores `agentIdCheckedAt` on the token; if more than 10 minutes have elapsed (or the field is missing at all, covering every session that pre-dates this fix), it silently re-queries `Agent` and updates both fields. Confirmed with Ryan this bounds the added DB cost to ~1 small query per active user per 10 minutes regardless of traffic — negligible — versus re-checking every request (defeats the July fix) or purely-reactive-only (never proactively self-corrects). Design doc: `docs/superpowers/specs/2026-07-25-agentid-session-refresh-design.md`.

**Verification throughout:** full suite went 544 → 601 tests, all passing at every step; `tsc --noEmit` clean; dev server log checked for hot-reload/runtime errors after every change (none found). Ryan confirmed live afterward that tab-switching "does feel much faster now."

### Next Session — Start Here

1. Run `pnpm --filter web dev` from `C:\Users\hey_r\Desktop\CnC-Realty`.
2. **Push `feature/agent-application-redesign`** whenever ready — 24 commits ahead of origin now (18 from before, plus tonight's 5 commits + the campaigns copy fix, none pushed yet).
...
## Session Notes — 2026-09-11 / 2026-09-12 (footer polish + corporate-lease/E&O business discussion)

### Website/email polish, all committed and pushed

- Website footer address line abbreviated: "830 S Main Street, STE 227" → "830 S. Main St, STE 227" (email footer intentionally left unabbreviated, Ryan's explicit call — the two footers no longer read identically on this one line, by design).
- Website footer reordered: the email link now renders above the address block (previously address was above email). A new inline-SVG `AddressIcon` component was added to `Footer.tsx`, matching the `currentColor` convention already used by `EmailIcon`/`FacebookIcon`/etc. (recolored from the source SVG's navy `#1C274C`), placed on the address's first line only, mirroring the email link's icon+text pattern.
- Footer social icons (Facebook/Instagram/YouTube) now hover to the same gold `#9E8C61` used by the nav links and email link, via a one-class Tailwind swap (`hover:text-white` → `transition-colors hover:text-[#9E8C61]`); the existing Framer Motion scale-up hover animation was untouched (separate mechanism, no conflict). Verified by reading the compiled CSS rule directly rather than trusting a screenshot, after Puppeteer's hover call proved unreliable in this environment yet again (same class of flakiness noted in the 2026-08-25/26 session).
- `/join` page pricing card: "Professional" renamed to "Pro"; the separate "Forever" line was merged into the price line so it now reads "$0/month forever" as one line.
- All of the above pushed to `origin/main` across two commits (`fccb117`, `e4be0fc`).
- Incidental: running a full `pnpm build` against the same `.next` directory the dev server was using broke the running dev server (stale/404'd static assets) — not a code bug, just don't `rm -rf .next` while the dev server is live; restart the dev server after a manual build check.

### Corporate lease line — discussed, decided to hold off

Ryan raised the idea of a dedicated website tab for corporate/office leases (more profitable than residential rentals) but has never personally done one himself. Talked through it as a business question, not a code task:

- **Liability**: higher stakes than residential, but not from licensure — a CA salesperson license already covers commercial deals, no separate license needed. The real risk is complexity/dollar exposure (NNN/CAM charges, TI allowances, personal guarantees, permitted-use/zoning, ADA compliance) and a more sophisticated counterparty than a typical renter.
- **Decision (Ryan's call): don't build a dedicated page yet.** The technical lift is smaller than it looks — CnC's transaction wizard already has a Residential/Commercial toggle, a Commercial 90/10 fee split is already defined, and ~6,000 commercial listings already sit in the DB, currently deliberately filtered out of search (see "Deferred — Commercial Listings" earlier in this file) — but marketing a service before Ryan has closed one deal risks generating leads for something he isn't yet ready to execute competently on. Validate demand/readiness first (word-of-mouth, or a low-cost mention on the existing Rent/Contact flow) before committing engineering time to a dedicated tab. Same "prove it before building it" logic as the earlier Client Portal decision.

### E&O insurance research — real findings, no carrier chosen yet

Picked up the "E&O Carrier Research — Deprioritized" thread from an earlier session (above) — this time actual carrier/product research, not just competitor fee-schedule comparison. All web-sourced and cited, not from memory. Ryan is independently looking into CRES himself as of this session; no decision made yet.

- **Not legally required in California — confirmed against primary sources**, not insurance-marketing blogs: the DRE's own Reference Book ("Brokers should consider carrying general liability and Errors and Omission insurance..." — recommendation language, not a mandate) and Business & Professions Code Division 4 (§§10000–10452, the actual licensing statute), which contains no E&O requirement. California's substitute is a DRE-administered Consumer Recovery Account (B&P §§10474–10476) paying wronged consumers up to $50,000/transaction. This doesn't make E&O optional for CnC in practice, though — the ICA already promises agents "E&O included through $1M sale price," a contractual commitment independent of the legal baseline. Two practical (non-legal) gates still worth checking directly: MLS membership commonly requires proof of E&O (unconfirmed specifically for CRMLS — ask them), and lenders/title companies often require proof before closing.
- **Best cheap option found: CRES Insurance's California "Express" program** — eligibility is ≤$150K annual revenue AND ≤5 licensees AND ≥80% of revenue from residential activity (1–4 units). This is a CA-specific tier, separate from CRES's other "Express" program ($600K revenue/25 licensees, doesn't operate in CA — don't let CRES quote against the wrong tier). No published price; needs a real quote. Exceeding any of the three eligibility limits doesn't cut CnC off from CRES — it moves to CRES's standard, individually-underwritten firm policy (a "Quick Quote"), which isn't a publicly comparable price, so nobody can promise CRES stays cheapest past that point — re-shop against E&O For Less/OREP (both explicitly serve up to 25 licensees with published, non-quoted pricing) once CnC approaches those limits.
- **CRES also offers a Per-Transaction (PT) payment plan** — billed monthly based on actual closed transactions rather than a flat rate, the model Ryan said he prefers. Caveats: not truly $0 in a zero-transaction month (a minimum charge for one transaction/month applies regardless), a higher minimum retention/deductible ($2,500+ starting) than a standard annual policy, and a security deposit (2x the per-transaction rate) to bind coverage. Unconfirmed whether PT billing is available on the cheap Express-tier rate or only on the standard firm policy — ask CRES directly.
- **Competitor carriers are not publicly disclosed anywhere** — confirmed again for Rise Realty (nothing public at all) and eXp Realty (discloses only its fee structure — **$60/transaction risk management fee, capped at $750/year** — not the underlying carrier). That eXp fee structure is a useful benchmark to bring into CnC's own quote conversations regardless of the carrier being unknown. Victor Insurance Managers came up as a large NAR-partnered real estate E&O underwriter (46,000+ policyholders) but nothing ties it to eXp specifically — an unconfirmed lead only, not a fact.

### Next Session — Start Here

1. Run `pnpm --filter web dev` from `C:\Users\hey_r\Desktop\CnC-Realty`
2. **Follow up on CRES** — Ryan was looking into CRES's Express program directly as of this session; find out what he learned (actual quote, whether PT billing is available on the Express tier, whether it covers what CnC needs) and whether a carrier decision got made.
3. **Deploy-day checklist item still open**: after `NEXTAUTH_URL` is updated to the real production domain, send a fresh test welcome email and confirm the logo/footer icons render correctly in an external client (Outlook, Gmail, etc.), not just via a same-machine webmail view.
