...
async function checkCampaignAccess(campaignId: string, agentId: string | null, role: string) {
  const campaign = await prisma.campaign.findUnique({ where: { id: campaignId } });
  if (!campaign) return { campaign: null, forbidden: false };
  if (role === "ADMIN") return { campaign, forbidden: false };
  return { campaign, forbidden: !agentId || campaign.agentId !== agentId };
}
...
export async function GET(_req: Request, { params }: Params) {
  ...
  const { campaign, forbidden } = await checkCampaignAccess(params.id, session.user.agentId, session.user.role);
