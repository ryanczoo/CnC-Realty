...
function ResetForm() {
  ...
  if (success) {
    return (
      <div className="text-center">
        <p className="font-sans text-xl font-light text-[#1B1B1B]">Password reset!</p>
  ...
  return (
    <div className="w-full max-w-sm space-y-5">
      <h1 className="font-sans text-2xl font-light text-[#1B1B1B]">Choose a new password</h1>
      <p className="text-sm text-[#1B1B1B]/50">This link expires 2 hours after it was sent.</p>
      {error && <div className="rounded-lg bg-red-50 px-4 py-3 text-sm text-red-600">{error}</div>}
      <div>
        <label className="mb-1 block text-xs font-medium uppercase tracking-wide text-[#1B1B1B]/60 text-left">New Password</label>
        <input
      ...
      <div>
        <label className="mb-1 block text-xs font-medium uppercase tracking-wide text-[#1B1B1B]/60 text-left">Confirm New Password</label>
        <input
          className={inputClass}
          type="password"
          value={confirm}
